 ...
 *   13  3         send msg to oneself use internal buffer
 *   13  4         send msg use internal buffer contain a struct data
 *   15  x         get the some status
 *--------------------------------------------------------------------
 ********************************************************************/
/********************************************************************
 * Include Files
...
eat_bool eat_module_test_sys(u8 param1, u8 param2)
{
    ...
    else if (15 == param1)
    {
        ...
        value = eat_network_get_ber();
        eat_trace("eat_network_get_ber=%d", value);

        value = eat_network_get_csq();
        eat_trace("eat_network_get_csq=%d", value);

        value = eat_network_get_cgatt();
        eat_trace("eat_network_get_cgatt=%d", value);
    }
    else if( 16 == param1 )
    {
		if( 0 == param2)
